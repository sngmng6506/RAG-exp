...
# 필터 인덱스 로드
# ===============================
def load_filter_indices():
    """틀린 문항 인덱스 로드 (filter_by_incorrect=True일 때만)"""
    if not CURRENT_CONFIG.filter_by_incorrect:
        return None
    
    filter_path = CURRENT_CONFIG.incorrect_indices_path
...
# Loaders
# ===============================
def load_dataset_from_jsonl(filter_indices=None) -> Dataset:
    ...
    with jsonl_path.open("r", encoding="utf-8") as f:
        for line_idx, line in enumerate(f):
            if line.strip():
                # 필터 인덱스가 있으면 해당 인덱스만 로드
                if filter_indices and line_idx not in filter_indices:
                    continue
...
def main():
    ...
    print(f"필터 모드: {cfg.filter_by_incorrect}")
    print(f"출력 경로: {cfg.output_dir}\n")
    
    # 필터 인덱스 로드
    filter_indices = load_filter_indices()
    if filter_indices:
        print(f"🎯 [필터 모드] {len(filter_indices)}개 문항만 평가")
        print(f"   인덱스: {sorted(filter_indices)}\n")
    
    # 1) JSONL에서 데이터셋 로드
    dataset = load_dataset_from_jsonl(filter_indices)
    ...
    out_csv = cfg.output_dir / f"ragas_results_{timestamp}.csv"
